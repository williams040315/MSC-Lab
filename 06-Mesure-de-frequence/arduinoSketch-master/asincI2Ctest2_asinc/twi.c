...
/*
 * Function twi_AsincWriteNotReady (added Lesto)
 * Desc     return the statu of the write operation
 * Input    length: number of bytes in output array
 * Output   0 .. success
 *          1 .. length to long for buffer
...
uint8_t twi_AsincWriteNotReady(){
  if (!twi_writing)
    return 5;//error, no reading asked
    
  if (TWI_MTX == twi_state)
    return 6; //error, waiting for right status
...
/* 
 * Function twi_status (added by Lesto)
 * Desc     return the status of the TWI 
 * Input    none
 * Output   twi status, see TWI.h
 */
uint8_t twi_status()
{
  return twi_state;
