...
##############################################################################################################################################
ser             = serial.Serial(port='COM12',baudrate = 9600,parity=serial.PARITY_NONE,stopbits=serial.STOPBITS_ONE,bytesize=serial.EIGHTBITS,timeout=2000)
...
def getConfig():
    ...
    global n, pos_absolue, vitesse_aller, vitesse_retour, t1, t2, t3
    n               = int(float(data_config[0]))                               
    pos_absolue     = int(uniform(int(float(data_config[1])),int(float(data_config[5]))))
    vitesse_aller   = int(float(data_config[2]))                               
    vitesse_retour  = int(float(data_config[6]))                               
    t1              = float(data_config[4])
    t2              = float(int(uniform(float(data_config[3]),float(data_config[7]))))
    t3              = float(data_config[8])
