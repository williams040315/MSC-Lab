...
async function optionSettingsChannel(user_email, select_option = null)
{
    var res = "";
    ...
    for (var i = 0; i < userSettingsChannels.length; i++) {
        console.log(userSettingsChannels[i]._id);
        res += "<option name='"+ userSettingsChannels[i].channel_name  + "' value='" + userSettingsChannels[i]._id + "' " + (select_option == userSettingsChannels[i]._id ? "selected" : "") + ">"
        res += userSettingsChannels[i].channel_name
        res += "</option>"
    }
...
router.get('/get-settings-channels', async (req, res) => {
    //console.log("\n\nheader\n\n",req.headers);
    ...
    // }
    //var otherResponse = listOption(checkStuff);
    //var lol = await Settings_channel.find( { channel_author: userToken.email} );
    var result = await optionSettingsChannel(userToken.email, req.headers.select_option);
