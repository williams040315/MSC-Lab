...
class TRACK_METH():
    ...
    def __init__(self):
        ...
        self.simple_cntr = np.array([[2,2], [7,1], [15,5], [25,25], [10,35]])
        #self.rec_perms = open('..processings')

    def swap(self,ind, ind_min, debug=[2]):
        '''
        swapping ind and ind_min
        '''
        if 1 in debug: print(f'swapping {ind} with {ind_min}')
        if 2 in debug: print(f'** {ind_min} goes at place {ind}')
    ...
    def far_contours(self):
        ...
        self.curr_contours.append(rand_cntr) # random contour very far away
    ...
    def bijective_nearest(self,ind,p,debug=0):
        ...
        ind_min_rev = self.find_nearest_index_reverse(ind_min)    # must be equal to ind, bijective
        if debug>0: print(f'## ind {ind}, ind_min {ind_min}, ind_min_rev {ind_min_rev}')
    ...
    def adapt_lists(self, l_prev, l_curr, debug=[]):
        ...
        '''
        if 1 in debug: print(f"type(l_curr[0]) is {type(l_curr[0])}")
        if isinstance(l_curr[0], tuple):
            self.orig_type_l_curr = 'tuple'
            if 2 in debug: print("found the type tuple !!!")
    ...
    def translate_to_tuple(self):
        '''
        Pass from list  of numpy arrays to list of tuples for track_segmentation format
        '''
    ...
    def pass_back_to_tuple(self, debug=0):
        ...
        if self.orig_type_l_curr == 'tuple':
            try:
                if debug>0:
                    print('#**## passing back values of self.list_prev_pos and self.list_pos ')
                    print(f'**** self.list_pos is at the end {self.list_pos} ')
                self.list_prev_pos, self.list_pos = self.translate_to_tuple()
            except:
                print('cannot pass from array to tuple at the end of the tracking step')

    def meth_min(self, l_prev, l_curr, corr=False, debug=0):
        '''
        ...
        for ind,p in enumerate(self.l_prev): # previous points
            ...
            if debug>0: print(f'{ind}/{ind_min}')
    ...
    def lcurr_shorter(self, debug=0):
        ...
        for i,j in enumerate(self.row_ind):
            ...
            try:
                self.sorted_list_cntrs[j] = copy.deepcopy(self.curr_contours[i])
            except:
                if debug>0:
                    print('no contours')
            ...
            if j not in self.col_ind:
                for k in range(len(self.l_curr)):
                    if isinstance(self.sorted_list[k],int):  # k == 0
                        ...
                        try:
                            self.sorted_list_cntrs[k] = copy.deepcopy(self.curr_contours[j])
                        except:
                            if debug>0:
                                print('no contours')
                        break

    def lcurr_longer(self, debug=0):
        '''
        '''
        print('### longer !!')
        ...
        #self.adapt_size()
        if debug == 1: print((f"### self.row_ind, self.col_ind {self.row_ind, self.col_ind} "))
        for i,j in enumerate(self.col_ind):
            self.sorted_list[i] = copy.deepcopy(self.l_curr[j])                     # old points
            try:
                self.sorted_list_cntrs[i] = copy.deepcopy(self.curr_contours[j])
            except:
                if debug>0:
                    print('no contours')
        self.insert_new_points()  # adding the new points at the end

    def assign_with_hung_mat(self, show_mat=False, debug=1):
        '''
        '''
        hung_mat = []
        for p in self.l_prev:                              # previous points
            ...
            for q in self.l_curr:                          # new points
                ldist += [ norm(p-q)**2 ]
            hung_mat += [ ldist ]
        if show_mat: print(f"hung_mat { hung_mat }")
        self.row_ind, self.col_ind = linear_sum_assignment(hung_mat)
        if self.show_assign: print(f"## row_ind, col_ind {self.row_ind, self.col_ind}")
    ...
    def meth_hung(self, l_prev, l_curr, corr=False, show_assign=False, debug=[0]):
        '''
        Hungarian method
        '''
        ...
        if corr:
            if 1 in debug: print(f'## self.l_curr before {self.l_curr}')
            self.assign_with_hung_mat()
            if diff < 0:
                self.lcurr_shorter()
            else:
                self.lcurr_longer()
            if 2 in debug: print(f"len(self.sorted_list), len(self.l_curr) {len(self.sorted_list), len(self.l_curr)}")
            self.apply_permutations_correction()    # change the initial vectors..
            if 1 in debug:
                print(f'## self.l_curr after {self.l_curr}')
            if 3 in debug: self.debug_changes_found()
        self.pass_back_to_tuple()
